'''
    <<< About this network >>>
    Deeplab v3 plus implementation for A.I.Segmentation
...
from keras.layers import UpSampling2D                   # Added for AIS, Takashi Shirakawa, Jan, 2020
from keras.layers.advanced_activations import ReLU      # Added for AIS, Takashi Shirakawa, Dec, 2019
...
# Removed for AIS, Takashi Shirakawa, Jan, 2020
...
# Change for AIS, Takashi Shirakawa, Dec, 2019
def relu6(x):
...
def Deeplabv3(backbone='xception', OS=16, alpha=1.0):
    ...
    img_input = ZeroPadding2D(4)(inputs)
    ...
    b4 = AveragePooling2D(pool_size=(int(208/OS), int(208/OS)))(x)
    b4 = Conv2D(256, (1, 1), padding='same', use_bias=False, name='image_pooling')(b4)
    b4 = BatchNormalization(name='image_pooling_BN', epsilon=1e-5)(b4)
    b4 = Activation('relu')(b4)
    b4 = UpSampling2D(size=(int(208/OS), int(208/OS)), interpolation='bilinear')(b4)
    # b4 = BilinearUpsampling(upsampling=(int(np.ceil(input_shape[0] / OS)), int(np.ceil(input_shape[1] / OS))))(b4)        # Change for AIS, Takashi Shirakawa, Jan, 2020
    ...
    if backbone == 'xception':
        b1 = SepConv_BN(x, 256, 'aspp1', rate=atrous_rates[0], depth_activation=True, epsilon=1e-5)     # rate = 6 (12)
        b2 = SepConv_BN(x, 256, 'aspp2', rate=atrous_rates[1], depth_activation=True, epsilon=1e-5)     # rate = 12 (24)
        b3 = SepConv_BN(x, 256, 'aspp3', rate=atrous_rates[2], depth_activation=True, epsilon=1e-5)     # rate = 18 (36)
        x = Concatenate()([b4, b0, b1, b2, b3])     # concatenate ASPP branches & project
    else:
        x = Concatenate()([b4, b0])
    ...
    if backbone == 'xception':
        # Feature projection
        # x4 (x2) block
        x = UpSampling2D(size=(int(OS/4), int(OS/4)), interpolation='bilinear')(x)
        # x = BilinearUpsampling(...)(x)    # Change for AIS, Takashi Shirakawa, Jan, 2020
    ...
    # if classes == 21: last_layer_name = 'logits_semantic'
...
def Build_Model():
    return Deeplabv3(backbone='xception', OS=16, alpha=1.0)
